...
const CONFIG = {
  ...
  siteName: process.env.ZENDESK_SITE_NAME || "Engage Knowledge Web",
};
...
// List all categories and sections
async function listCategories(): Promise<string> {
  try {
    ...
    let output = "# Engage Knowledge Web Categories\n\n";
...
async function searchArticles(query: string): Promise<string> {
  try {
    ...
    let output = `# Search Results for "${query}"\n\nFound ${results.length} article(s):\n\n`;
...
const tools: Tool[] = [
  {
    ...
    description: "List all available categories and sections in Engage Knowledge Web. Use this to discover what topics and help articles are available.",
    inputSchema: {
      type: "object",
      properties: {},
    },
  ...
  {
    name: "fetch_article",
    description: "Fetch and return the full content of a specific article by its ID. Use this to get the complete, up-to-date content of an article.",
    inputSchema: {
      type: "object",
      properties: {
        article_id: {
  ...
  {
    name: "search_content",
    description: "Search for articles in Engage Knowledge Web. Returns matching articles with previews and IDs.",
    inputSchema: {
      type: "object",
      properties: {
        query: {
